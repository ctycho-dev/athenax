   ...
   return (
      <aside className="h-full w-[280px] flex flex-col border-r border-gray-2">
         <div className="flex-1 flex flex-col">
            <div className="p-6 flex gap-1.5 items-center">
...
const NavbarItems = () => {
   ...
   return (
      <ul className="text-sm font-normal text-gray-3">
         {navbar.filter(item => item.display).map((item, index) => {
            return (
               <li key={`navbar-${item.id}`}>
                  <Link to={item.path}
                     className={`navbar-item flex justify-between px-sm py-xs mb-sm transition-all duration-150 hover:text-[#0092ff] rounded-[6px] hover:cursor-pointer hover:bg-[rgb(0,146,255,0.1)]
            ${location.pathname === `${item.path}` ? 'bg-[rgb(0,146,255,0.1)] text-[#0092ff] active' : ''}`}
                  >
                     <div className='grid grid-cols-[16px_1fr] items-center gap-x-[8px]'>
                        <span
